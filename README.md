...
- **`PLAN`**: 부위별 `main`(직렬·긴 휴식: 스트렝스 2.5분/하이퍼 2분) / `ss`(슈퍼세트 페어, 튜플 `[lab, arr, restSec?, note?]`) / `solo`(단독) / `order`(표시 순서).
- **슈퍼세트 안전 규칙**: **같은 근육은 묶지 않음**(코어는 허용 — push/pull은 마지막 보조+코어를 묶어 휴식 시간 절약). 로드 시 `validateProgram`이 order 누락/중복·같은근육 슈퍼세트를 **런타임 검증 → UI 배너 + 콘솔 에러**. 안전 기준은 *같은 근육 X* **AND** *전신·호흡 고부하 2개 동시 X* — 후자는 코드 규칙이 아니라 PLAN 배치로 관리(레그의 스쿼트/RDL/BSS/스윙은 원칙상 단독). 단 **BSS+케틀벨스윙은 사용자 선택으로 슈퍼세트**(과거 어지럼 사고 페어 → 라운드 휴식 120s + ⚠ 경고문구로 완화).
- **영상**: 모든 종목은 **@DeltaBolic 특정 쇼츠**(`vid`)로 직링크(`vidHref`). 워밍업은 영상 없음.
